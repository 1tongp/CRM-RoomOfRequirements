...
function CustomerList(props) {
	...
	useEffect(
		() => {
			axios.get('/customer/show/' + props.data.location.state.staff.id).then(response => {
	...
	for (let i = 0; i < data.length; i++) {
		data[i].details.push(<CustomerDetail className='button' data={data[i]}>Details</CustomerDetail>)
	}
	...
	for (let i = 0; i < data2.length; i++) {
		data2[i].details.push(<CustomerDetail className='button' data={data2[i]}>Details</CustomerDetail>)
		data2[i].assign.push(<CustomerAssign className='button' staff = {props.data.location.state.staff.id} data={data2[i]}>Assign</CustomerAssign>)
	}
	...
	var filteredData2 = data2.filter(function (el) {
		return result2.includes(el.firstName) || result2.includes(el.lastName) ||
			result2.includes(el.contactNumber.toString()) || result2.includes(el.email);
	});
		...
		<>
			<div className='total'>
				...
				<div>
					...
					<Table size='middle' className='table' columns={columns} dataSource={filteredData} onChange={onChange} />
				</div>
			...
			<div className='total'>
				...
				<div>
					...
					<Table size='small' className='table' columns={columns2} dataSource={filteredData2} onChange={onChange} />
				</div>
