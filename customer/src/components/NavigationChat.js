...
class Navigation extends React.Component {
  ...
  handleClick = e => {
    ...
    if(e.key === '2'){
        ...
        axios.get('/customer/list/' + this.props.data.state.staff.id).then(response =>{
            console.log(response);
            if(!response.data.success){
              this.props.data.query.history.push('/customer', {staff: this.props.data.state.staff, customers:[], key:'2'})
            }
            else{
              this.props.data.query.history.push('/customer', {staff: this.props.data.state.staff, customers: response.data.customers, key:'2'});
            }
          })
    }
    if(e.key === "1"){
      ...
      axios.post('/staff/login/unhash', { loginEmail: this.props.data.state.staff.loginEmail , password: this.props.data.state.staff.password }).then(response =>{
        console.log(response);
        if(!response.data.success){
          this.props.data.query.history.push('/dashboard', {staff: response.data.staff, key:'1'})
        }
        else{
          this.props.data.query.history.push('/dashboard', {staff: response.data.staff, key:'1'});
        }
    ...
    if(e.key === "5"){
      ...
      this.props.data.query.history.push('/ProfileShow', {staff: this.props.data.state.staff, key: '5'});
    }
    if(e.key === "4"){
      console.log("switch to calendar");
      console.log(this.props);
      // axios needs implement later after finishing the back-end of the Caldenlar
      this.props.data.query.history.push('/calendar', {staff: this.props.data.state.staff, key: '4'});
    }
    if(e.key === "3"){
      console.log("switch to group and chat");
      console.log(this.props);
      this.props.data.query.history.push('/group', {staff: this.props.data.state.staff, key: '3'});
  ...
  onLogOut = () =>{
    console.log("log out");
    console.log(this.props);
    this.props.data.query.history.push('../');
  }
